...
GoogleSpeech::GoogleSpeech(QString language)
{
...
void GoogleSpeech::setLanguage(QString language)
{
    m_language = language;
}

void GoogleSpeech::speech(QString text)
{
    if (QMultimedia::Available == 0) {
        QString i;
        text.replace(" ","+");
        m_playlist->addMedia(QUrl(m_url.arg(m_language).arg(text)));
        qDebug() << QUrl(m_url.arg(m_language).arg(text));
...
void GoogleSpeech::errorSlot()
{
    emit error();
}

void GoogleSpeech::state(QMediaPlayer::State state)
{
